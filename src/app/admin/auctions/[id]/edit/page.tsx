...
import {
  ChevronLeft, Save, Plus, Trash2, Gavel, DollarSign,
  Calendar, Shield, Users, Image as ImageIcon, Loader2
} from 'lucide-react';
  ...
  useEffect(() => {
    async function fetchAuction() {
      try {
        ...
        setCourtCode(data.courtCode || '');
        setAddress(data.address || '');
        setAddressDetail(data.addressDetail || '');
        setCity(data.city || '');
        setDistrict(data.district || '');
        setItemType(data.itemType || 'APARTMENT');
        setLandArea(data.landArea?.toString() || '');
        setBuildingArea(data.buildingArea?.toString() || '');
        setFloor(data.floor || '');

        // 가격 정보
        setAppraisalPrice(data.appraisalPrice || '');
        setMinimumPrice(data.minimumPrice || '');
        setMinimumRate(data.minimumRate?.toString() || '');
        setDeposit(data.deposit || '');

        // 일정
        setSaleDate(formatDateForInput(data.saleDate));
        setSaleTime(data.saleTime || '');
        setBidCount(data.bidCount?.toString() || '1');
        setBidEndDate(formatDateForInput(data.bidEndDate));

        // 권리분석
        setReferenceDate(formatDateForInput(data.referenceDate));
        setHasRisk(data.hasRisk || false);
        setRiskNote(data.riskNote || '');
        ...
        if (data.registers && data.registers.length > 0) {
          setRegisters(data.registers.map((r: Record<string, unknown>) => ({
            id: r.id,
            registerType: r.registerType || '을',
            registerNo: r.registerNo || '',
            receiptDate: formatDateForInput(r.receiptDate as string | null),
            purpose: r.purpose || '',
            rightHolder: r.rightHolder || '',
            claimAmount: r.claimAmount || '',
            isReference: r.isReference || false,
            willExpire: r.willExpire !== false,
            note: r.note || '',
          })));
        }
        ...
        if (data.tenants && data.tenants.length > 0) {
          setTenants(data.tenants.map((t: Record<string, unknown>) => ({
            id: t.id,
            tenantName: t.tenantName || '',
            hasPriority: t.hasPriority || false,
            occupiedPart: t.occupiedPart || '',
            moveInDate: formatDateForInput(t.moveInDate as string | null),
            fixedDate: formatDateForInput(t.fixedDate as string | null),
            hasBidRequest: t.hasBidRequest || false,
            deposit: t.deposit || '',
        ...
        if (data.images && data.images.length > 0) {
          setImages(data.images.map((img: Record<string, unknown>) => ({
            id: img.id,
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">경매 물건 수정</h1>
          <p className="text-gray-600">{caseNumber}</p>
        </div>
      </div>
